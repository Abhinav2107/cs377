        ...
        for (i = 0; i < dirSize; ++i) {
            ...
            if (strcmp(entry->fileName, path) == 0) {
                ...
                if (!rest) {
                    ...
                    Block_Read(mountPoint->dev, entry->firstBlock, currDir);
                    path = rest;
                    break;
                }
            }
...
static int PFAT_Open(struct Mount_Point *mountPoint, const char *path,
    ...
    file =
        Allocate_File(&s_pfatFileOps, 0, entry->fileSize, pfatFile, mode, 0);
    if (file == 0) {
        rc = ENOMEM;
        goto done;
    }
...
static int PFAT_Open_Directory(struct Mount_Point *mountPoint,
    ...
    if (strcmp(path, "/") == 0) {
        ...
        *pDir = dir;
        ...
        directoryEntry *items = (directoryEntry *) Malloc(entry->fileSize);
 ...
 */
static int PFAT_Sync(struct Mount_Point *mountPoint __attribute__ ((unused))) {
...
static int PFAT_Mount(struct Mount_Point *mountPoint) {
    ...
    if (fsinfo->fileAllocationOffset <= 0 ||
        ...
        fsinfo->rootDirectoryCount < 0 || fsinfo->rootDirectoryOffset <= 0) {
